...
@interface LYNumberScrollView ()
//跑马灯数组
@property (nonatomic , strong) NSMutableArray *btnArray;

@end

@implementation LYNumberScrollView
    ...
    if (self) {
        ...
        if (titlesArray.count>0) {
            for (int i = 0; i < titlesArray.count ; i++) {
                ...
                [imagesBtn setTitleColor:RGB(251, 105, 12) forState:0];
                imagesBtn.backgroundColor = [UIColor whiteColor];
                imagesBtn.titleLabel.font = PFR14Font;
                ...
                [imagesBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                    [make.left.mas_equalTo(self.imageView.mas_right)setOffset:DCMargin];
                    make.centerY.mas_equalTo(self);
                    make.size.mas_equalTo(CGSizeMake(60, 20));
                }];
                
                [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                    [make.left.mas_equalTo(imagesBtn.mas_right)setOffset:DCMargin];
                    make.centerY.mas_equalTo(self);
