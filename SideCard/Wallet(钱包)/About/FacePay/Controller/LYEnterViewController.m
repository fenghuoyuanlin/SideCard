//
//  LYEnterViewController.m
//  GuanruAppDemo
//
//  Created by 刘园 on 2017/12/19.
...
@interface LYEnterViewController ()<UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate>
//tableView
...
@implementation LYEnterViewController
...
-(void)setUpTopAndBottom
{
    ...
    [_continueBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(420 - 15 + 40 - 135);
...
-(void)setUpData
{
    _registerArr = [LYRegisterItem mj_objectArrayWithFilename:@"EnterData.plist"];
}
...
-(void)continueBtnClick
{
    ...
    if (![DCSpeedy isBlankString:cellOne.textField.text] && ![DCSpeedy isBlankString:cellTwo.textField.text] && ![DCSpeedy isBlankString:cellThree.textField.text] && ![DCSpeedy isBlankString:cellFour.textField.text] && ![DCSpeedy isBlankString:cellFive.textField.text])
    {
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [hud hideAnimated:YES];
            [DCSpeedy alertMes:@"提交成功,待审核"];
        });
    }
    else
    {
